package org.trishullab.gcflobdd.api.jdd;
...
import org.trishullab.gcflobdd.GCFLOBDD;
...
public class BDD {
   ...
   protected static final int CACHE_FORALL = 1;
    ...
    private final GCFLOBDD.Grammar grammar;
    private final int level;
    private int varCount;
    private List<GCFLOBDD> bddtable;
    ...
    public BDD(int level, int cache_size, GCFLOBDD.Grammar grammar) {
        this.level = level;
        ...
        lookup = new HashMap<>();
        bddtable.add(GCFLOBDD.createFalse(level, grammar));
        bddtable.add(GCFLOBDD.createTrue(level, grammar));
        refCount.add(1);
        refCount.add(1);
    ...
    private int putBdd(GCFLOBDD val) {
    ...
    public int createVar() {
        GCFLOBDD bdd = GCFLOBDD.createVar(varCount++, level, grammar);
        return putBdd(bdd);
    }
    ...
    public int not(int bdd) {
        ...
        GCFLOBDD notBdd = GCFLOBDD.not(bddtable.get(bdd));
    ...
    public int and(int bdd1, int bdd2) {
        ...
        GCFLOBDD andBdd = GCFLOBDD.and(bddtable.get(bdd1), bddtable.get(bdd2));
    ...
    public int or(int bdd1, int bdd2) {
        ...
        int hash = this.op_cache.hash_value;
        GCFLOBDD orBdd = GCFLOBDD.or(bddtable.get(bdd1), bddtable.get(bdd2));
    ...
    public int imp(int bdd1, int bdd2) {
        ...
        int hash = this.op_cache.hash_value;
        GCFLOBDD impBdd = GCFLOBDD.imp(bddtable.get(bdd1), bddtable.get(bdd2));
    ...
    public int exists(int bdd, int var) {
        //TODO: exist cache
        GCFLOBDD existsBdd = GCFLOBDD.exists(bddtable.get(bdd), var);
    ...
    public void cleanup() {
        GCFLOBDD.dispose_module();
    }

    public static void main(String[] args) {
        GCFLOBDD.Grammar grammar = GCFLOBDD.createGrammar(new String[] {"S 1 -> S 0 S 0", "S 0 -> a"}, "S 1");
        BDD bdd = new BDD(10, 100000, grammar);
        int a = bdd.createVar();
